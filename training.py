...
#Outputs: min_asl.path - best checkpoint
...
# Pytorch device, uses APPLE GPU, or NVIDIA, or CPU
def device():
...
def tfms_train(sz: int, aug: bool):
    ...
    ops += [
        transforms.ToTensor(), #changes image to number values
...
# images foe validation, no augmentation is done 
def tfms_val(sz: int):
...
# model - the network, loader - loads images, dev - Apple GPU, criterion - loss funciton
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if va_acc > best_accuracy:
            ...
            #  save labels for web/demo reference
